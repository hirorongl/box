...
define ('NEXT_SCRIPT', 'userbox/rpfile.php');
...
//この追加フィールドについての表示制限チェック
//0表示する（orderに指定可能）
//1ログインユーザのみ表示する
//2グループ(所有者含)とadmin権のある人のみ表示 
//3所有者とadmin権のある人のみ表示 
//4admin権のある人のみ表示 
//5表示しない 
$allow_display=DB_getItem($_TABLES['USERBOX_def_field'],"allow_display","field_id={$id}");
if  ($allow_display==0){
}elseif ($allow_display==1 AND !COM_isAnonUser()) {	
}else{
    $display .= databox_siteHeader($pi_name,'',$page_title);
    $display .= databox_siteFooter($pi_name);
    COM_output($display);
